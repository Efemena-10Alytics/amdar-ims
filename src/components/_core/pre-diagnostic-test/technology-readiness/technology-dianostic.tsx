...
import TechnologyDiagnosticDrawer from "./technology-diagnostic-drawer";
...
const TechnologyDiagnostic = () => {
  ...
  return (
    <section className="w-full max-w-190 px-4 pb-5 pt-0 sm:px-0 sm:pb-8">
      <h1 className="text-2xl font-semibold text-[#173740]">Technology Diagnostics</h1>
      ...
      <TechnologyDiagnosticDrawer
        open={isDiagnosticOpen}
        onOpenChange={setIsDiagnosticOpen}
        onComplete={() => router.push("/pre-diagnostic-test/ims-readiness")}
      />
    </section>
  );
};

export default TechnologyDiagnostic;
