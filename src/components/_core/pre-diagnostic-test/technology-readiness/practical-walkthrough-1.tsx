...
const PracticalWalkthrough1 = () => {
  const router = useRouter();
  const { preDiagnostic } = usePreDiagnosticNavigation();
  const [hasVideoEnded, setHasVideoEnded] = useState(false);

  const walkthroughVideo = preDiagnostic.technology_readiness.PracticalWalkthrough[0];
  ...
  return (
    <section className="w-full max-w-190 px-4 pb-5 pt-0 sm:px-0 sm:pb-8">
      <h1 className="text-2xl font-semibold text-[#173740]">Practical walkthrough 1</h1>
...
export default PracticalWalkthrough1;
