...
const CareerKnowledgeDiscovery1 = () => {
  const router = useRouter();
  const { preDiagnostic } = usePreDiagnosticNavigation();
  const [hasVideoEnded, setHasVideoEnded] = useState(false);

  const discoveryVideo = preDiagnostic.career_readiness.careerKnowledgeDiscovery[0];
  ...
  return (
    <section className="w-full max-w-190 px-4 pb-5 pt-0 sm:px-0 sm:pb-8">
      ...
      <button
        ...
        onClick={() =>
          router.push("/pre-diagnostic-test?step=career-knowledge-discovery-2")
        }
...
export default CareerKnowledgeDiscovery1;
