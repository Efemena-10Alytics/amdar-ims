"use client";
import { useEffect } from "react";
import { initClassicAos } from "./init-classic-aos";

type MySpecializationProps = {
  specializations?: string[];
  softSkills?: string[];
  id?: string;
};

export function MySpecialization({
  specializations = [],
  softSkills = [],
  id,
}: MySpecializationProps) {
  useEffect(() => {
    initClassicAos();
  }, []);
  return (
    <section id={id} data-aos="fade-up" className="mt-20" aria-label="Specialization and skills">
      {/* My Specialization */}
      <div className="text-lg font-semibold text-[#A1A8B1] mb-4">
        My Specialization
      </div>
      <ul className="grid max-w-6xl mx-auto grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-10 list-none">
        {specializations.map((item, index) => (
          <li key={index} className="flex items-center justify-center gap-2">
            <span
              className="shrink-0 size-1.5 rounded-full bg-primary"
              aria-hidden
            />
            <h3 className="text-base font-semibold text-[#092A31] font-clash-display">{item}</h3>
          </li>
        ))}
      </ul>

      {/* Soft skills */}
      <h3 data-aos="fade-up" className="text-lg font-extrabold text-[#E8EFF1] text-center mt-20 mb-6">
        Soft skills
      </h3>
      <div data-aos="fade-up" className="flex flex-wrap max-w-2xl mx-auto gap-4 justify-center">
        {softSkills.map((skill, index) => (
          <span
            key={index}
            className="rounded-full bg-[#F8FAFC] px-3 py-1.5 text-base font-medium text-[#64748B]"
          >
            {skill}
          </span>
        ))}
      </div>
    </section>
  );
}
