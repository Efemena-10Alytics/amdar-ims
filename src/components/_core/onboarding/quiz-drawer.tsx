...
const QuizDrawer = ({
  ...
  const selectedValue = activeQuestion ? (answers[activeQuestion.id] ?? "") : "";
  ...
  useEffect(() => {
    if (!open || secondsLeft <= 0 || totalQuestions === 0) return;
    const timer = window.setInterval(() => {
      setSecondsLeft((prev) => Math.max(prev - 1, 0));
    }, 1000);

    return () => window.clearInterval(timer);
  }, [open, secondsLeft, totalQuestions]);

  const handleContinue = () => {
    if (!activeQuestion || !selectedValue) return;
